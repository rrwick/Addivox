...
def copy_resources_to_destination(projectpath, dst, label=""):
  ...
  if os.path.exists(projectpath + "/resources/presets/"):
    ...
    shutil.copytree(projectpath + "/resources/presets/", presets_dst, dirs_exist_ok=True)
...
# AUDIOUNIT v3
  ...
  if config['PLUG_HAS_UI'] == 1:
    ...
    auv3['NSExtension']['NSExtensionAttributes']['AudioComponents'][0]['factoryFunction'] = "IPlugAUViewController_vAdditiveWindSynth"
    auv3['NSExtension']['NSExtensionMainStoryboard'] = config['BUNDLE_NAME'] + "-iOS-MainInterface"
  else:
    auv3['NSExtension']['NSExtensionPrincipalClass'] = "IPlugAUViewController_vAdditiveWindSynth"
