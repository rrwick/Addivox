...
void effects::Chorus::ProcessBlock(iplug::sample** outputs, int nFrames)
{
  ...
  for(int sampleIndex = 0; sampleIndex < nFrames; ++sampleIndex)
  {
    ...
    outputs[0][sampleIndex] =
      static_cast<iplug::sample>(dsp::FlushDenormal(outputs[0][sampleIndex] + (wetLeft * parameters.wetMix)));
    outputs[1][sampleIndex] =
      static_cast<iplug::sample>(dsp::FlushDenormal(outputs[1][sampleIndex] + (wetRight * parameters.wetMix)));
  }
