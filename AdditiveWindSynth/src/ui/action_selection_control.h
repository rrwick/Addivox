...
public:
  ...
  ActionSelectionControl(const IRECT& bounds,
                         const char* defaultText,
                         const std::initializer_list<const char*>& menuItems,
                         const IText& text,
  ...
  void SetOnSelection(SelectionFunc func)
  {
    mOnSelection = std::move(func);
  }

  void OnMouseDown(float x, float y, const IMouseMod& mod) override
  {
    if(IsDisabled() || !(mod.L || mod.R) || !GetUI())
      return;
  ...
  void OnPopupMenuSelection(IPopupMenu* pSelectedMenu, int valIdx) override
  {
    ...
    if(pSelectedMenu && pSelectedMenu->GetChosenItem())
      selectedText = pSelectedMenu->GetChosenItem()->GetText();

    if(selectedText && mOnSelection)
      mOnSelection(selectedText);
