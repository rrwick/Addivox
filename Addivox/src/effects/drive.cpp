...
constexpr std::array<double, 12> kOversamplingCoefs2x{
    0.036681502163648017, 0.13654762463195794, 0.27463175937945444, 0.42313861743656711, 0.56109869787919531, 0.67754004997416184,
...
double effects::Drive::EvaluateShaperAdaa(ChannelState& channel, double input, const Parameters& parameters) {
  ...
  if (!channel.shaperStateInitialized) {
    ...
    if (std::abs(delta) <= kAdaaDeltaThreshold) {
      ...
      saturated = (EvaluateShaperAntiderivative(input, parameters) - EvaluateShaperAntiderivative(previousInput, parameters)) / delta;
    }
  }
