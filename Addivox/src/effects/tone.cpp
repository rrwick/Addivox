...
constexpr std::array<double, 8> kCrossoverFrequenciesHz{80.0, 160.0, 320.0, 640.0, 1280.0, 2560.0, 5120.0, 10240.0};
constexpr std::array<double, 9> kBandOctaveOffsets{-4.5, -3.5, -2.5, -1.5, -0.5, 0.5, 1.5, 2.5, 3.5};
} // namespace
...
double effects::Tone::ComputeTrimForAmount(double amount) const {
  ...
  for (int probeIndex = 0; probeIndex < kTrimProbeCount; ++probeIndex) {
    ...
    const double probeFrequencyHz = kTrimProbeMinimumFrequencyHz * std::pow(maxFrequencyHz / kTrimProbeMinimumFrequencyHz, frequencyT);
...
double effects::Tone::LookupTrim(double amount) const {
  const double tablePosition = 0.5 * (std::clamp(amount, -1.0, 1.0) + 1.0) * static_cast<double>(kTrimTableSize - 1);
...
void effects::Tone::UpdateCrossoverCoefficients() {
  for (std::size_t index = 0; index < mCrossoverCoefficients.size(); ++index) {
    mCrossoverCoefficients[index] =
        dsp::CutoffHzToCoefficient(mSampleRate, kCrossoverFrequenciesHz[index], kTrimProbeMinimumFrequencyHz, kCrossoverMaximumScale);
  }
}
...
bool effects::Tone::HasStoredSignal() const {
  for (const auto& channel : mChannels) {
    for (const auto& crossover : channel.crossovers) {
      if (crossover.state != 0.0) return true;
    }
  }
