...
inline bool DeserializeKeyNoteEqCurvePayload(int dataSize, const void* pData, int& midiNote, EqCurve& curve) {
  if (dataSize < static_cast<int>(sizeof(int) * 2) || !pData) return false;
  ...
  if (position < 0 || numPoints < 0) return false;
