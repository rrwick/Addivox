...
public:
  ...
  enum class ValueTransform { Linear, SquareRoot, PseudoLog };
  ...
  explicit OscillatorSliderControl(const IRECT& bounds, const char* label = "", const IVStyle& style = DEFAULT_STYLE,
                                   EDirection direction = EDirection::Vertical)
      : Base(bounds, label, style, 0, direction) {
    SetOnNewValueFunc([this](int oscillatorIndex, double normalizedValue) { HandleSliderValueChanged(oscillatorIndex, normalizedValue); });
    ApplyBaseValue();
  }
  ...
  void SetConfig(const Config& config) {
    ...
    mConfig = config;
    if (!std::isfinite(mConfig.range.min) || !std::isfinite(mConfig.range.max) || mConfig.range.max <= mConfig.range.min) mConfig.range = ValueRange{};
  ...
  void SetOnOscillatorValueChanged(OnOscillatorValueChangedFunc func) { mOnOscillatorValueChanged = std::move(func); }

  void SetOscillatorValue(int oscillatorIndex, double value) { SetValue(ToControlValueFromRangeValue(value), oscillatorIndex); }

  double GetOscillatorValue(int oscillatorIndex) const { return FromControlValueToRangeValue(GetValue(oscillatorIndex)); }
  ...
  void SetOscillatorEditableFunc(IsOscillatorEditableFunc func) { mIsOscillatorEditable = std::move(func); }

  void SetOscillatorEditModeFunc(GetOscillatorEditModeFunc func) { mGetOscillatorEditMode = std::move(func); }
  ...
  void SetVisibleOscillatorRange(int minOscillatorOneBased, int maxOscillatorOneBased) {
    ...
    if (maxOscillator < minOscillator) maxOscillator = minOscillator;

    if (mVisibleOscillatorMin == minOscillator && mVisibleOscillatorMax == maxOscillator) return;
  ...
  void OnResize() override {
    ...
    if (readoutBandHeight <= 0.f) return;
  ...
  void DrawTrackHandle(IGraphics& g, const IRECT& r, int chIdx, bool aboveBaseValue) override {
    ...
    if (alignedRect.W() <= 0.f || alignedRect.H() <= 0.f) return;
  ...
  void SnapToMouse(float x, float y, EDirection direction, const IRECT& bounds, int valIdx = -1, double minClip = 0., double maxClip = 1.) override {
    ...
    case EditorOscillatorEditMode::Nudge:    changedValue = HandleNudgeDrag(sliderHit, controlValue); break;
    case EditorOscillatorEditMode::Smooth:   changedValue = HandleSmoothDrag(sliderHit, controlValue); break;
    case EditorOscillatorEditMode::DrawLine: changedValue = HandleDrawLineDrag(sliderHit, controlValue); break;
    case EditorOscillatorEditMode::Set:
    default:                                 changedValue = HandleSetDrag(sliderHit, controlValue); break;
    }
...
private:
  ...
  static double ClampSignedUnit(double value) { return std::clamp(value, -1.0, 1.0); }

  static float FloorToPixelBoundary(float value, float scale) {
    return static_cast<float>(std::floor(static_cast<double>(value) * static_cast<double>(scale)) / static_cast<double>(scale));
  }

  static float CeilToPixelBoundary(float value, float scale) {
    return static_cast<float>(std::ceil(static_cast<double>(value) * static_cast<double>(scale)) / static_cast<double>(scale));
  }

  static float RoundToPixelBoundary(float value, float scale) {
    return static_cast<float>(std::round(static_cast<double>(value) * static_cast<double>(scale)) / static_cast<double>(scale));
  }

  float GetBackingPixelScale() const {
    if (const IGraphics* ui = GetUI()) return std::max(1.f, ui->GetTotalScale());
  ...
  static std::pair<float, float> GetPixelAlignedLengthParts(float length, float scale) {
    ...
    if (fractional <= 1.0e-6) fractional = 0.0;
    else if ((1.0 - fractional) <= 1.0e-6) {
      wholePixels += 1.0;
      fractional = 0.0;
    }

    return {static_cast<float>(wholePixels / static_cast<double>(scale)), static_cast<float>(fractional)};
  }
  ...
  static float GetPositiveEdgeCoverage(float edge, float scale) {
    ...
    if (fractional <= 1.0e-6 || fractional >= 1.0 - 1.0e-6) return 0.f;
  ...
  static float GetNegativeEdgeCoverage(float edge, float scale) {
    ...
    if (coverage <= 0.f) return 0.f;

    return 1.f - coverage;
  }

  float GetTipCoverage(const IRECT& r, bool aboveBaseValue, float scale) const {
    if (r.W() <= 0.f || r.H() <= 0.f) return 0.f;

    if (mDirection == EDirection::Horizontal) return aboveBaseValue ? GetPositiveEdgeCoverage(r.R, scale) : GetNegativeEdgeCoverage(r.L, scale);
  ...
  IRECT ExtendRectOutward(const IRECT& anchorRect, float length, bool aboveBaseValue) const {
    if (length <= 0.f) return IRECT();

    if (mDirection == EDirection::Horizontal) {
      if (aboveBaseValue) return IRECT(anchorRect.R, anchorRect.T, anchorRect.R + length, anchorRect.B);

      return IRECT(anchorRect.L - length, anchorRect.T, anchorRect.L, anchorRect.B);
    }

    if (aboveBaseValue) return IRECT(anchorRect.L, anchorRect.T - length, anchorRect.R, anchorRect.T);

    return IRECT(anchorRect.L, anchorRect.B, anchorRect.R, anchorRect.B + length);
  }

  void DrawBipolarBarFill(IGraphics& g, const IRECT& originalRect, bool aboveBaseValue, const IColor& color) const {
    const float scale = GetBackingPixelScale();
    const float tipThickness = 1.f / scale;

    // Keep a stable 2-pixel core centered on zero so exact-zero and near-zero bipolar values stay visually ordered while the value grows outward.
    ...
    if (tipCoverage > 0.f) {
      ...
      if (tipRect.W() > 0.f && tipRect.H() > 0.f) DrawPixelAlignedTip(g, tipRect, ScaleColorOpacity(color, tipCoverage));
    }
  }

  void DrawPixelAlignedTip(IGraphics& g, const IRECT& tipRect, const IColor& color) const {
    if (tipRect.W() <= 0.f || tipRect.H() <= 0.f) return;

    // Use a line rather than a tiny rectangle so the partially covered final pixel row/column gets uniform alpha across the bar width.
    if (mDirection == EDirection::Horizontal) {
  ...
  void DrawBarFill(IGraphics& g, const IRECT& originalRect, const IRECT& alignedRect, bool aboveBaseValue, const IColor& color) const {
    const float scale = GetBackingPixelScale();
    ...
    const float tipThickness = std::min(1.f / scale, mDirection == EDirection::Horizontal ? alignedRect.W() : alignedRect.H());
    IRECT bodyRect = alignedRect;
    IRECT tipRect = alignedRect;
    SplitAlignedTipRect(bodyRect, tipRect, aboveBaseValue, tipThickness);

    if (bodyRect.W() > 0.f && bodyRect.H() > 0.f) g.FillRect(color, bodyRect, &mBlend);

    if (tipRect.W() > 0.f && tipRect.H() > 0.f) DrawPixelAlignedTip(g, tipRect, ScaleColorOpacity(color, tipCoverage));
  }

  IRECT GetAlignedFillRect(const IRECT& r, bool aboveBaseValue) const {
    if (r.W() <= 0.f || r.H() <= 0.f) return IRECT();
  ...
  EditorOscillatorEditMode GetOscillatorEditMode() const { return mGetOscillatorEditMode ? mGetOscillatorEditMode() : EditorOscillatorEditMode::Set; }
  ...
  int FindOscillatorIndexForPoint(float x, float y, EDirection direction) const {
    ...
    for (int oscillatorIndex = 0; oscillatorIndex < nVals; ++oscillatorIndex) {
      ...
      const bool isHit = direction == EDirection::Vertical ? trackBounds.ContainsEdge(x, trackBounds.MH()) : trackBounds.ContainsEdge(trackBounds.MW(), y);
      if (isHit) return oscillatorIndex;
    }
  ...
  int FindNearestVisibleOscillatorIndex(float position, EDirection direction) const {
    ...
    for (int oscillatorIndex = 0; oscillatorIndex < nVals; ++oscillatorIndex) {
      ...
      const float trackCenter = direction == EDirection::Vertical ? trackBounds.MW() : trackBounds.MH();
  ...
  MouseEditPoint GetMouseEditPoint(float x, float y, EDirection direction, const IRECT& bounds, bool allowOutsideTrackSelection = false) const {
    ...
    if (direction == EDirection::Vertical) {
      if (step > -1) {
        ...
        if (mStepBounds.GetSize() == 1) value = 1.f;
        else
      ...
      if (sliderHit < 0 && allowOutsideTrackSelection) sliderHit = FindNearestVisibleOscillatorIndex(x, direction);
    } else {
      if (step > -1) {
      ...
      if (sliderHit < 0 && allowOutsideTrackSelection) sliderHit = FindNearestVisibleOscillatorIndex(y, direction);
    }

    if (!GetStepped()) value = std::round(value / mGrain) * mGrain;
  ...
  template <typename StepFunc> bool HandleStepwiseDrag(int sliderHit, double cursorControlValue, StepFunc&& applyStep) {
    if (sliderHit > -1) mMouseOverTrack = sliderHit;
    else
      mSliderHit = -1;

    if (sliderHit < 0) return false;
  ...
  bool HandleNudgeDrag(int sliderHit, double cursorControlValue) {
    return HandleStepwiseDrag(sliderHit, cursorControlValue,
                              [this](int oscillatorIndex, double stepCursorControlValue) { return ApplyNudgeStep(oscillatorIndex, stepCursorControlValue); });
  }

  bool HandleSmoothDrag(int sliderHit, double cursorControlValue) {
    return HandleStepwiseDrag(sliderHit, cursorControlValue, [this](int oscillatorIndex, double) { return ApplySmoothStep(oscillatorIndex); });
  }
  ...
  bool ApplyNudgeStep(int oscillatorIndex, double cursorControlValue) {
    ...
    if (cursorControlValue > currentControlValue + kValueComparisonTolerance) nudgedControlValue = Clamp01(currentControlValue + kNudgeControlStep);
  ...
  double GetSmoothedControlValue(int oscillatorIndex) const {
    ...
    for (int offset = -3; offset <= 3; ++offset) {
      ...
      if (neighbourIndex < 0 || neighbourIndex >= SimplePatch::kNumOscillators) continue;
      if (!IsOscillatorEditable(neighbourIndex)) continue;
    ...
    if (totalWeight <= 0.0) return GetValue(oscillatorIndex);
  ...
  bool ApplyDrawLinePreview(int endOscillatorIndex, double endControlValue) {
    ...
    if (startOscillatorIndex == endOscillatorIndex) {
      ...
      for (int oscillatorIndex = lowBounds; oscillatorIndex <= highBounds; ++oscillatorIndex) {
        ...
        desiredValues[static_cast<std::size_t>(oscillatorIndex)] = Clamp01(iplug::Lerp(startControlValue, endControlValue, frac));
      }
    }
  ...
  float GetReadoutBandHeight() const { return std::clamp(mRECT.H() * 0.06f, kReadoutMinBandHeight, kReadoutMaxBandHeight); }

  double ApplyTransform(double normalizedValue) const {
    const double clamped = Clamp01(normalizedValue);
    switch (mConfig.transform) {
    case ValueTransform::SquareRoot: return transformations::NormalizedSquareRoot(clamped);
    case ValueTransform::PseudoLog:  return transformations::NormalizedExpInverse(clamped, transformations::GetGlobalPseudoLogShapeValue());
  ...
  double InvertTransform(double controlValue) const {
    ...
    switch (mConfig.transform) {
    case ValueTransform::SquareRoot: return transformations::NormalizedSquareRootInverse(clamped);
    case ValueTransform::PseudoLog:  return transformations::NormalizedExp(clamped, transformations::GetGlobalPseudoLogShapeValue());
    case ValueTransform::Linear:
    default:                         return clamped;
    }
  }
  ...
  double ApplySignedTransform(double normalizedValue) const {
    ...
    switch (mConfig.transform) {
    case ValueTransform::SquareRoot: return transformations::SignedNormalizedSquareRoot(clamped);
    case ValueTransform::PseudoLog:  return transformations::SignedNormalizedExpInverse(clamped, transformations::GetGlobalPseudoLogShapeValue());
    case ValueTransform::Linear:
    default:                         return clamped;
    }
  }
  ...
  double InvertSignedTransform(double controlValue) const {
    ...
    switch (mConfig.transform) {
    case ValueTransform::SquareRoot: return transformations::SignedNormalizedSquareRootInverse(clamped);
    case ValueTransform::PseudoLog:  return transformations::SignedNormalizedExp(clamped, transformations::GetGlobalPseudoLogShapeValue());
    case ValueTransform::Linear:
    default:                         return clamped;
    }
  }
  ...
  bool UsesBipolarRange() const { return mConfig.range.min < 0.0 && mConfig.range.max > 0.0; }

  bool UsesMirroredBipolarTransform() const {
    if (!UsesBipolarRange()) return false;
  ...
  double GetMirroredBipolarMaxMagnitude() const { return std::max(std::fabs(mConfig.range.min), std::fabs(mConfig.range.max)); }
  ...
  double ToControlValueFromRangeValue(double value) const {
    ...
    if (!UsesMirroredBipolarTransform()) return ApplyTransform(Normalize(clampedValue));

    const double maxMagnitude = GetMirroredBipolarMaxMagnitude();
    if (maxMagnitude <= 0.0) return 0.5;
  ...
  double FromControlValueToRangeValue(double controlValue) const {
    ...
    if (!UsesMirroredBipolarTransform()) return Denormalize(InvertTransform(clampedControlValue));
  ...
  int GetReadoutOscillatorIndex() const {
    ...
    if (IsVisibleOscillatorIndex(mHighlightedTrack)) return mHighlightedTrack;
  ...
  IText MakeReadoutText(const char* font, float size, EAlign align, EVAlign valign) const { return IText(size, colour::ui::kValueText, font, align, valign); }
  ...
  WDL_String FormatOscillatorValue(int oscillatorIndex) const {
    ...
    if (absValue < 1e-12) text.Set("0.0");
  ...
  bool IsOscillatorEditable(int oscillatorIndex) const { return !mIsOscillatorEditable || mIsOscillatorEditable(oscillatorIndex); }
  ...
  void MakeTrackRects(const IRECT& bounds) override {
    ...
    for (int ch = 0; ch < nVals; ++ch) mTrackBounds.Get()[ch] = IRECT();
    ...
    for (int ch = visibleMin; ch <= visibleMax; ++ch) {
      ...
      IRECT trackBounds = bounds.SubRect(EDirection(!dir), visibleCount, visibleIndex)
                              .GetPadded(0, -mTrackPadding * static_cast<float>(dir), -mTrackPadding * static_cast<float>(!dir), -mTrackPadding);
      ...
      if (ch < visibleMax) {
        if (mDirection == EDirection::Horizontal) trackBounds.B = std::max(trackBounds.T, trackBounds.B - gapSize);
        else
