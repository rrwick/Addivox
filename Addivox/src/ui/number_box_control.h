...
{
public:
  NumberBoxControl(const IRECT& bounds,
                   int paramIdx,
  ...
  {
    if(!mNumberBox || !mIncrementButton || !mDecrementButton)
      return;
...
private:
  ...
  public:
    ...
    void StepValue(double direction)
    {
      ...
      const double snappedValue = GetParam() ? GetParam()->FromNormalized(GetParam()->ToNormalized(mRealValue))
                                             : mRealValue;
  ...
  private:
    void ApplyValueChange(bool preventAction = false)
    {
      ...
      if(GetParam())
        displayValue = GetParam()->FromNormalized(GetParam()->ToNormalized(mRealValue));
  ...
  public:
    ...
    : IVButtonControl(bounds, SplashClickActionFunc, "", style, false, false)
  ...
  void StepValue(double direction)
  {
    if(!mNumberBox || mNumberBox->IsDisabled())
      return;

    const bool hasParamBinding = mNumberBox->GetParam() && mNumberBox->GetDelegate();
    if(hasParamBinding)
      mNumberBox->GetDelegate()->BeginInformHostOfParamChangeFromUI(mNumberBox->GetParamIdx());

    mNumberBox->StepValue(direction);

    if(hasParamBinding)
      mNumberBox->GetDelegate()->EndInformHostOfParamChangeFromUI(mNumberBox->GetParamIdx());
  }
