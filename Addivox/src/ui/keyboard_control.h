...
public:
  ...
  KeyboardControl(const IRECT& bounds, int minNote = 48, int maxNote = 72) : IVKeyboardControl(bounds, minNote, maxNote) {}
  ...
  void SetHighlightedMidiNote(int midiNote, bool highlighted) {
    ...
    if (mHighlightedMidiNotes[midiNoteIndex] == highlighted) return;
  ...
  void SetSelectedMidiNote(int midiNote) {
    ...
    if (mOnSelectedMidiNoteChanged) mOnSelectedMidiNoteChanged(mSelectedMidiNote);
    SetDirty(false);
  }
...
private:
  ...
  bool IsMidiNoteInRange(int midiNote) const { return midiNote >= mMinNote && midiNote <= mMaxNote; }

  bool IsMidiNoteHighlighted(int midiNote) const {
    if (!IsValidMidiNote(midiNote)) return false;

    return mHighlightedMidiNotes[static_cast<size_t>(midiNote)];
  }

  bool IsMidiNoteSelected(int midiNote) const { return midiNote == mSelectedMidiNote; }

  void SelectNoteAtPoint(float x, float y) {
    const int keyIndex = GetKeyAtPointForEditMode(x, y);
    if (keyIndex >= 0) SetSelectedMidiNote(GetMidiNoteNumberForKey(keyIndex));
  }
  ...
  int GetKeyAtPointForEditMode(float x, float y) const {
    ...
    if (geometry.numKeys <= 0) return -1;

    for (int key = 0; key < geometry.numKeys; ++key) {
      if (!mIsBlackKeyList.Get()[key]) continue;
      ...
      if (keyBounds.Contains(x, y)) return key;
    }
  ...
  int CollectUpperOcclusions(float keyLeft, float keyRight, const KeyGeometry& geometry, std::array<Interval, kMaxUpperOcclusions>& occlusions) const {
    int numOcclusions = 0;
    for (int key = 0; key < geometry.numKeys; ++key) {
      if (!mIsBlackKeyList.Get()[key]) continue;
      ...
      if (numOcclusions < static_cast<int>(occlusions.size())) occlusions[static_cast<size_t>(numOcclusions++)] = {overlapLeft, overlapRight};
    }
  ...
  void DrawSelectedWhiteKeyFill(IGraphics& g, int key, const KeyGeometry& geometry, const IColor& fillColor) {
    ...
    if (geometry.blackKeyBottom < mRECT.B) g.FillRect(fillColor, {keyLeft, geometry.blackKeyBottom, keyRight, mRECT.B}, &mBlend);

    if (geometry.blackKeyBottom <= mRECT.T) return;
    ...
    std::sort(occlusions.begin(), occlusions.begin() + numOcclusions, [](const Interval& a, const Interval& b) { return a.l < b.l; });

    float visibleStart = keyLeft;
    for (int i = 0; i < numOcclusions; ++i) {
      const Interval& occlusion = occlusions[static_cast<size_t>(i)];
      if (occlusion.l > visibleStart) g.FillRect(fillColor, {visibleStart, mRECT.T, occlusion.l, geometry.blackKeyBottom}, &mBlend);

      visibleStart = std::max(visibleStart, occlusion.r);
    }

    if (visibleStart < keyRight) g.FillRect(fillColor, {visibleStart, mRECT.T, keyRight, geometry.blackKeyBottom}, &mBlend);
  }
  ...
  void DrawWhiteKeyNoteDot(IGraphics& g, int key, const KeyNoteDotGeometry& dotGeometry) {
    ...
    const float dotCenterY = std::clamp(mRECT.B - dotGeometry.bottomOffset, dotGeometry.whiteRegionTop + dotGeometry.radius, mRECT.B - dotGeometry.radius);
  ...
  void DrawBlackKeyNoteDot(IGraphics& g, const IRECT& keyBounds, const KeyNoteDotGeometry& dotGeometry) {
    const float dotCenterY = std::clamp(keyBounds.B - dotGeometry.bottomOffset, keyBounds.T + dotGeometry.radius, keyBounds.B - dotGeometry.radius);
    g.FillCircle(mKeyNoteDotColor, keyBounds.MW(), dotCenterY, dotGeometry.radius, &mBlend);
  }

  void EndMouseNoteIfNeeded() {
    if (mLastTouchedKey < 0) return;
  ...
  void DrawEditModeHighlights(IGraphics& g) {
    ...
    for (int key = 0; key < geometry.numKeys; ++key) {
      ...
      if (!isSelected && !isKeyNote) continue;

      if (isSelected) DrawSelectedWhiteKeyFill(g, key, geometry, selectedWhiteFillColor);

      if (isKeyNote) DrawWhiteKeyNoteDot(g, key, dotGeometry);
    }
    ...
    for (int key = 0; key < geometry.numKeys; ++key) {
      ...
      const float keyLeft = mKeyXPos.Get()[key];
      const IRECT keyBounds{keyLeft, mRECT.T, keyLeft + geometry.blackKeyWidth, geometry.blackKeyBottom};
      if (isSelected) g.FillRect(selectedBlackFillColor, keyBounds, &mBlend);

      if (isKeyNote) DrawBlackKeyNoteDot(g, keyBounds, dotGeometry);
    }
  }
