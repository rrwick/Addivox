...
transformations::PseudoLogExpShape::PseudoLogExpShape(double shape) : mShape(shape) {}

double transformations::NormalizedSquareRoot(double normalized) { return std::sqrt(std::clamp(normalized, 0.0, 1.0)); }
...
iplug::IParam::Shape* transformations::PseudoLogExpShape::Clone() const { return new PseudoLogExpShape(*this); }

iplug::IParam::EDisplayType transformations::PseudoLogExpShape::GetDisplayType() const { return iplug::IParam::kDisplayLog; }
...
double transformations::NormalizedExp(double normalized, double shape) {
  ...
  if (std::abs(shape) <= kShapeEpsilon) return x;

  const double denominator = std::expm1(shape);
  if (std::abs(denominator) <= kShapeEpsilon) return x;
...
double transformations::NormalizedExpInverse(double scaledValue, double shape) {
  ...
  if (std::abs(shape) <= kShapeEpsilon) return y;

  const double denominator = std::expm1(shape);
  if (std::abs(denominator) <= kShapeEpsilon) return y;
...
double transformations::GetGlobalPseudoLogShapeValue() { return kGlobalPseudoLogShape; }
