...
public:
  explicit HarmonicVisualizerControl(const IRECT& bounds) : IControl(bounds) {}
  ...
  void Draw(IGraphics& g) override {
    ...
    for (const float frequencyHz : kGridFrequenciesHz) {
      ...
      g.DrawLine(major ? plugin_ui::colour::visualizer::kGridMajor : plugin_ui::colour::visualizer::kGridMinor, x, barPlot.T, x, barPlot.B, &mBlend,
                 major ? 1.1f : 1.f);
    }
    ...
    for (const auto& osc : mFrame.harmonics) {
      if (osc.frequencyHz < kMinDrawFrequencyHz || osc.frequencyHz > kMaxDrawFrequencyHz || osc.level <= 0.f) continue;
      ...
      const IColor harmonicColor =
          LerpColor(plugin_ui::colour::visualizer::kHarmonicGradientStart, plugin_ui::colour::visualizer::kHarmonicGradientEnd, colorT);
...
private:
  struct LogFrequencyMapping {
    ...
    float Normalize(float frequencyHz) const { return NormalizeFromLog(std::log(std::clamp(frequencyHz, minHz, maxHz))); }

    float NormalizeFromLog(float logFrequency) const { return std::clamp((logFrequency - logMin) * invLogSpan, 0.f, 1.f); }
  };
  ...
  static float LerpXFromNormalized(float xNorm, const IRECT& bounds) { return bounds.L + (xNorm * bounds.W()); }
  ...
  static IColor LerpColor(const IColor& a, const IColor& b, float t) {
    ...
    return IColor(lerpChannel(a.A, b.A), lerpChannel(a.R, b.R), lerpChannel(a.G, b.G), lerpChannel(a.B, b.B));
  }
  ...
  static constexpr std::array<float, 13> kAxisLabelFrequenciesHz{20.f,   30.f,   50.f,   100.f,  200.f,   300.f,  500.f,
                                                                 1000.f, 2000.f, 3000.f, 5000.f, 10000.f, 20000.f};
  static constexpr std::array<const char*, 13> kAxisLabelStrings{"20 Hz", "30 Hz", "50 Hz", "100 Hz", "200 Hz", "300 Hz", "500 Hz",
