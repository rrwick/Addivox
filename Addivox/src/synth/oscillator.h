...
class Oscillator {
public:
  // This function generates the output of the oscillator (two values for left and right channels) and advances the internal state by one sample.
  ...
  double GetCurrentPitchSemitones() const { return mPitch; }
...
private:
  ...
  // Level envelope is just for attack and release shaping (decay and sustain
  // are controlled by the player's breath). Not strictly for the start/end of
  // notes but more generally for controlling how quickly the oscillator level
  // can increase or decrease. Smoothing is done with a simple one-pole
  // approach, so attack is exponential rise toward the target level and release
  ...
  // phase and level. Phase is advanced by an increment (determined by the
  ...
  // Stereo pan state. Uses constant-power pan law (-3 dB). Pan variation is
  // evaluated sample-accurately to avoid control-rate zipper noise; a separate
  // slew limiter still protects against abrupt pan jumps from UI or automation
  // changes.
  ...
  // The shape of variation (level, pitch, pan) over time is determined by
  // gradient noise, which is a smooth random function that is similar to Perlin
  // noise.
