...
enum class BreathCCSource : uint8_t
{
  ModWheel = 0,
  Breath,
  BreathHighRes,
...
inline const char* GetBreathCCSourceMenuLabel(BreathCCSource source)
{
  switch(source)
  {
    case BreathCCSource::ModWheel:
      return "CC 1 (Mod Wheel)";
    case BreathCCSource::Breath:
      return "CC 2 (Breath)";
    case BreathCCSource::BreathHighRes:
      return "CC 2 + CC 34 (High-Res Breath)";
    case BreathCCSource::Volume:
      return "CC 7 (Volume)";
    case BreathCCSource::VolumeHighRes:
      return "CC 7 + CC 39 (High-Res Volume)";
    case BreathCCSource::Expression:
      return "CC 11 (Expression)";
    case BreathCCSource::ExpressionHighRes:
      return "CC 11 + CC 43 (High-Res Expression)";
    default:
      return GetBreathCCSourceMenuLabel(kDefaultBreathCCSource);
  }
}
...
inline int GetBreathCCSourceMSBController(BreathCCSource source)
{
  switch(source)
  {
    ...
    case BreathCCSource::ExpressionHighRes:
      return 11;
    default:
      return GetBreathCCSourceMSBController(kDefaultBreathCCSource);
  }
}
...
{
  if(rawValue < 0 || rawValue >= static_cast<int>(BreathCCSource::kCount))
    return kDefaultBreathCCSource;
...
inline std::optional<BreathCCSource> DetectBreathCCSource(
  const std::array<bool, 128>& seenControllers)
{
  if(seenControllers[2] && seenControllers[34])
    return BreathCCSource::BreathHighRes;
  if(seenControllers[2])
    return BreathCCSource::Breath;
  if(seenControllers[11] && seenControllers[43])
    return BreathCCSource::ExpressionHighRes;
  if(seenControllers[11])
    return BreathCCSource::Expression;
  if(seenControllers[7] && seenControllers[39])
    return BreathCCSource::VolumeHighRes;
  if(seenControllers[7])
    return BreathCCSource::Volume;
  if(seenControllers[1])
    return BreathCCSource::ModWheel;
...
public:
  ...
  BreathCCValueUpdate HandleMessage(BreathCCSource source, const iplug::IMidiMsg& msg)
  {
    ...
    if(msg.StatusMsg() != iplug::IMidiMsg::kControlChange)
      return update;
